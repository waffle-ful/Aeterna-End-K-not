...
All notable changes to End K not are recorded here. For older history (changes inherited from upstream EHR), see the [EHR repository](https://github.com/Gurge44/EndlessHostRoles).
...
- Rebrand: Plugin GUID and mod name changed to `EndKnot` / "End K not"
...
- BGM system: in-game background music replacement (work in progress); default tracks are bundled in the DLL; hosts may override per slot with their own files
