...
[BepInPlugin(PluginGuid, "End K not", BepInLoaderVersion)]
...
public class Main : BasePlugin
{
    ...
    public const string PluginVersion = "0.9.8.6-beta";
    // BepInEx は [BepInPlugin] の version を SemVer 2.0.0 (コアは必ず MAJOR.MINOR.PATCH の3要素)
    // で解析し、失敗時のみ System.Version (サフィックス不可) にフォールバックする。公開版と開発版の
    // 合間に臨時版を出すときの "0.9.8.5-beta" のような4要素+サフィックスはどちらにも通らず、
    // BepInEx が型ごと無音でスキップして未読み込み (バニラ起動) になる。
    // 4要素の場合だけ4要素目を prerelease のドット区切り識別子へ落として SemVer 合法化する
    // ("0.9.8.5-beta" → "0.9.8-beta.5")。3要素はそのまま (release.ps1 が bump 時に両方書き込む)。
    // PluginVersion 自体はホスト/クライアント互換性判定 (Version.Split('-')[0]) や表示に使うため
    // 4要素表記のまま変えない — BepInEx へ渡す文字列だけをこちらで別に持つ。
    public const string BepInLoaderVersion = "0.9.8-beta.6";
    ...
    public static HashSet<byte> LobbyKillers = [];
    public static HashSet<byte> LobbyDead = [];
    ...
    public static ConfigEntry<bool> EnableBGM { get; private set; }
    public static ConfigEntry<bool> ShowBGMInfo { get; private set; }
    public static ConfigEntry<float> BGMVolume { get; private set; }
    ...
    public static ConfigEntry<bool> EnableBoehmCensus { get; private set; }
    public static ConfigEntry<bool> SlimVanillaTextures { get; private set; }
    public static ConfigEntry<bool> ShowStartupFadeIn { get; private set; }
    public static ConfigEntry<bool> BgmMemorySaver { get; private set; }
    public static ConfigEntry<bool> BgmUseAssetBundle { get; private set; }
    public static ConfigEntry<bool> SfxUseAssetBundle { get; private set; }
    public static ConfigEntry<bool> LoadAllLanguages { get; private set; }
    ...
    public static ConfigEntry<bool> EosBootMarks { get; private set; }
    public static ConfigEntry<bool> DisableIncrementalGc { get; private set; }
    ...
    public static ConfigEntry<bool> BatchedPatching { get; private set; }
    public static ConfigEntry<bool> DelegateTypeCache { get; private set; }
    ...
    public override void Load()
    {
        ...
        try
        {
            ...
            {
                string src = Path.Combine(root, legacy);
                if (Directory.Exists(src) && !Directory.Exists(dst))
                {
                    Directory.Move(src, dst);
                    Log.LogInfo($"Migrated {legacy} -> EndKnot_DATA");
                    break;
                }
            }
            string cfgDst = Path.Combine(Paths.ConfigPath, "EndKnot.cfg");
            foreach (string legacy in new[] { "com.gurge44.endlesshostroles.cfg", "KnotHost.cfg" })
            {
                string src = Path.Combine(Paths.ConfigPath, legacy);
                if (File.Exists(src) && !File.Exists(cfgDst))
                {
                    File.Move(src, cfgDst);
                    Log.LogInfo($"Migrated {legacy} -> EndKnot.cfg");
                    break;
                }
            }
        }
        catch (Exception e) { Log.LogWarning($"Migration warning: {e.Message}"); }
        ...
        EnableBGM = Config.Bind("Client Options", "EnableBGM", true);
        ShowBGMInfo = Config.Bind("Client Options", "ShowBGMInfo", true);
        BGMVolume = Config.Bind("Client Options", "BGMVolume", 0.7f);
        ...
        BgmMemorySaver = Config.Bind("Client Options", "BgmMemorySaver", true, "Decode BGM tracks at half sample rate (44.1kHz -> 22.05kHz, stereo kept) to halve their resident PCM memory (each track is 33-50MB as 44.1kHz float PCM; 1-4 tracks stay resident). Default on for low-spec hosts. Turn off for full-bandwidth BGM; takes effect for tracks decoded after the change (restart to re-decode everything).");
        BgmUseAssetBundle = Config.Bind("Client Options", "BgmUseAssetBundle", true, "Play BGM from the bundled AssetBundle (Vorbis kept compressed in memory, decoded natively by the engine) instead of decoding the embedded OGG files into float PCM. Cuts BGM memory from 25-50MB per resident track to about 1-2MB. Turn off to fall back to the OGG decode path. Windows and Android; on Android the OGG files are not shipped, so the bundle path stays on regardless of this setting.");
        SfxUseAssetBundle = Config.Bind("Client Options", "SfxUseAssetBundle", true, "Play the long sound effects (WaveCannon charge/fire, Backrooms lobby ambience) from the bundled AssetBundle (Vorbis kept compressed in memory) instead of decoding them into float PCM. Saves about 15MB of resident memory. Turn off to fall back to the PCM decode path. Windows and Android.");
        LoadAllLanguages = Config.Bind("Client Options", "LoadAllLanguages", false, "Load every bundled language table at startup instead of only English plus the active language. Default off (saves ~30MB); turn on when editing translations or switching languages often.");
        ...
        MapAtmosphereFlash = Config.Bind("Client Options", "MapAtmosphereFlash", true, "Full-screen white flash when lightning strikes in the map atmosphere, and the dark flicker after close thunder. Turn off to keep the bolts without either.");
        ...
        EosBootMarks = Config.Bind("Debug", "EosBootMarks", true, "Record the vanilla EOS login-chain milestones (inventory refresh, stats init, login flow end) as BOOT marks (eos.*) so the time between reaching the main menu and being allowed to host can be attributed. Host-local timing only; four small Harmony postfixes.");
        ...
        DeferredPatching = Config.Bind("Client Options", "DeferredPatching", true, "Apply in-game Harmony patches during the splash screen instead of during startup (faster boot). Turn off if the main menu misbehaves; combine with BatchedPatching=false and DelegateTypeCache=false for the plain PatchAll path.");
        BatchedPatching = Config.Bind("Client Options", "BatchedPatching", true, "Compile each patched game method once per startup phase instead of once per patch class (faster boot). Turn off if a patch silently stops working.");
        DelegateTypeCache = Config.Bind("Client Options", "DelegateTypeCache", true, "Reuse one delegate type per signature for Harmony native trampolines instead of loading a new assembly per patched method (much faster boot). Affects every Harmony user in the process. Turn off if patches fail to apply.");
        ...
        try
        {
            RoleHtmlColors = new()
            {
                ...
                // Vanilla Remakes
                { CustomRoles.CrewmateEndKnot, "#8cffff" },
                { CustomRoles.EngineerEndKnot, "#FF6A00" },
                { CustomRoles.ScientistEndKnot, "#8ee98e" },
                { CustomRoles.GuardianAngelEndKnot, "#77e6d1" },
                { CustomRoles.TrackerEndKnot, "#34ad50" },
                { CustomRoles.NoisemakerEndKnot, "#ff4a62" },
                { CustomRoles.DetectiveEndKnot, "#625EEE" },
                // Crewmates
                ...
                { CustomRoles.Banker, "#DAA520" },
                { CustomRoles.Monochromer, "#808080" },
                { CustomRoles.Turncoat, "#F0E68C" },
                { CustomRoles.Remotekiller, "#DC143C" },
                { CustomRoles.Strawdoll, "#7b4122" },
                { CustomRoles.CurseMaker, "#554d59" },
                { CustomRoles.Missioneer, "#b1ae8f" },
                ...
                { CustomRoles.Exorcist, "#b59410" },
                { CustomRoles.Frightener, "#d35400" },
                { CustomRoles.Obstructer, "#5a7a8c" },
                { CustomRoles.Operative, "#47f5d2" },
                { CustomRoles.Blockade, "#d6fa34" },
                { CustomRoles.Jackpot, "#00fd50" },
                { CustomRoles.Shadow, "#4b3a66" },
                ...
                { CustomRoles.Amnesia, "#a0a0a0" },
                { CustomRoles.ToiletFan, "#a7dcfa" },
                { CustomRoles.Android, "#87ceeb" },
    ...
    private static void HandleRoleColorFiles()
    {
        ...
        if (!Directory.Exists($"{DataPath}/EndKnot_DATA"))
            Directory.CreateDirectory($"{DataPath}/EndKnot_DATA");

        var path = $"{DataPath}/EndKnot_DATA/RoleColors.json";
