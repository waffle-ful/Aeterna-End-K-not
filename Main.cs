...
public class Main : BasePlugin
{
    ...
    public override void Load()
    {
        ...
        SlimVanillaTextures = Config.Bind("Client Options", "SlimVanillaTextures", true, "Shrink and compress a few oversized vanilla textures at runtime once the main menu is reached (TextureSlimmer): the judge-gavel animation frames (hammerSlam_01..06, 6 x 9MB uncompressed RGBA32, ~54MB resident for the whole session) are re-initialised in place at half resolution + BC3 (about 0.6MB each). Sprite references stay valid so the animation keeps working. Set false to A/B (the result is logged as TEXSLIM in EndKnot-Health.log and visible in CENSUSTOP kind=tex).");
