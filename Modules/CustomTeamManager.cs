...
internal static class CustomTeamManager
{
    ...
    public static void LoadCustomTeams()
    {
        try
        {
            if (!File.Exists($"{Main.DataPath}/EndKnot_DATA/CTA_Data.txt")) return;

            CustomTeams = File.ReadAllLines($"{Main.DataPath}/EndKnot_DATA/CTA_Data.txt").Select(x => new CustomTeam(x)).ToHashSet();
    ...
    private static void RefreshCustomOptions()
    {
        ...
        static CustomTeamOptionGroup CreateSetting(CustomTeam team, int id)
        {
            ...
            if (HtmlColor.TryParse(team.RoleRevealScreenBackgroundColor, out Color color)) enabled.SetColor(color);
