        ...
        try
        {
            ...
            switch (rpc)
            {
                case RpcCalls.CheckName:
                {
                    if (!GameStates.IsLobby)
                    {
                        ...
                        Logger.Fatal($"Player [{pc.OwnerId}:{pc.GetRealName()}] CheckName out of lobby, rejected (no ban)", "EAC");
                        sr.Recycle();
                        return true;
                    }
            ...
            switch (callId)
            {
                ...
                case unchecked((byte)420): // 164 Sicko
                {
                    ...
                    {
                        Report(pc, "Sicko RPC (Hack against host-only mods, like EndKnot)");
                        HandleCheat(pc, "Sicko RPC (Hack against host-only mods, like EndKnot)");
