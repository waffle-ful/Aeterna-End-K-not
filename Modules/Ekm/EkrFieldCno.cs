...
// 見た目・生成戦略は EkrCno と同じ (Shapeshift-text 戦略・<size=N> 絶対値のみ・memory:
// cno_size_absolute_mode_policy) だが、EkrCno は sealed かつ「OnMeeting() を意図的に override しない」
// (会議明け自動復活エンジンに乗る) 契約を持つクラスなので、field 専用に別クラスを起こす — field は
// 「持続終了・会議開始・ホルダー死亡で実体ごと消滅」で、会議明け自動復活エンジンには乗せない
// (spec §3 v1.3・EkrDummyCno.OnMeeting 空 override と同型の裁定)。
internal sealed class EkrFieldCno : CustomNetObject, IEkrSlotCno
{
    ...
    // 一括で Despawn する (EkrDummyCno.OnMeeting 空 override と同じ裁定・二重管理防止)。
