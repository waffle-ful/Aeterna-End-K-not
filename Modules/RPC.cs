    ...
    public static void Postfix(PlayerControl __instance, [HarmonyArgument(0)] byte callId, [HarmonyArgument(1)] MessageReader reader)
    {
        try
        {
            ...
            switch (rpcType)
            {
                ...
                case CustomRPC.SyncLobbyState:
                {
                    int killerCount = reader.ReadPackedInt32();
                    Main.LobbyKillers = [];
                    for (int i = 0; i < killerCount; i++) Main.LobbyKillers.Add(reader.ReadByte());
                    int deadCount = reader.ReadPackedInt32();
                    Main.LobbyDead = [];
...
internal static class RPC
{
    ...
    public static void SyncLobbyState()
    {
        ...
        Logger.Info($"SyncLobbyState sending: Killers=[{string.Join(",", Main.LobbyKillers)}] Dead=[{string.Join(",", Main.LobbyDead)}]", "LobbyKill");
        MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.SyncLobbyState, SendOption.Reliable, -1);
        writer.WritePacked(Main.LobbyKillers.Count);
        foreach (byte id in Main.LobbyKillers) writer.Write(id);
        writer.WritePacked(Main.LobbyDead.Count);
        foreach (byte id in Main.LobbyDead) writer.Write(id);
        AmongUsClient.Instance.FinishRpcImmediately(writer);
    }
