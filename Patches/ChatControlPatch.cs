...
public static class ChatManager
{
    ...
    public static void SendMessage(PlayerControl player, string message)
    {
        ...
        {
            { } str when CheckCommand(ref str, "id|guesslist|gl编号|玩家编号|玩家id|id列表|玩家列表|列表|所有id|全部id|shoot|guess|bet|st|gs|bt|猜|赌|sp|jj|tl|trial|审判|判|审|xp|效颦|效|颦|sw|换票|换|swap|pick", false) || CheckName(ref playername, "系统消息", false) => 1,
