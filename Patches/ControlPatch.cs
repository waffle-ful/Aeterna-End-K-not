        ...
        try
        {
            ...
            if (HudManager.InstanceExists)
            {
                ...
                if (GameStates.IsLobby && (!HudManager.Instance.Chat || !HudManager.Instance.Chat.IsOpenOrOpening))
                {
                    ...
                    if (KeysDown(KeyCode.Return) && GameSettingMenu.Instance && GameSettingMenu.Instance.isActiveAndEnabled && !NumericOptionInputPatch.IsInputOpen)
...
public static class InGameRoleInfoMenu
{
    ...
    private static void Init()
    {
        ...
        Fill = new("EndKnot Role Info Menu Fill") { layer = 5 };
        ...
        Menu.name = "EndKnot Role Info Menu Page";
    ...
    public static void SetRoleInfoRef(PlayerControl player)
    {
        ...
        if (player.UsesMeetingShapeshift() && !MeetingTargetPicker.WouldGetButton(player)) sb.Append($"\n\n<size=85%>{GetString("UsesMeetingShapeshiftNotice")}</size>");
