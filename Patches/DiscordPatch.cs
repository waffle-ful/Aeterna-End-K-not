...
public static class DiscordRPC
{
    ...
    public static void Prefix([HarmonyArgument(0)] Activity activity)
    {
        ...
        var details = $"{Main.ModName} v{Main.PluginDisplayVersion}";
        ...
        try
        {
            if (activity.State != "In Menus")
            {
                if (!DataManager.Settings.Gameplay.StreamerMode)
                {
                    ...
                    if (Lobbycode != "" && Region != "") details = $"{Main.ModName} - {Lobbycode} ({Region})";
                }
                else
                    details = $"{Main.ModName} v{Main.PluginDisplayVersion}";
