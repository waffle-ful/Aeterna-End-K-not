...
// ロビー内マッチ情報パネル (MatchInfoGuide) に MOD の設定・役職一覧を表示する。クライアント側描画のみ・RPC 無し。
// Ported from upstream Gurge44/EndlessHostRoles e71fd450f + 151f0694e (2026-08-21).
[HarmonyPatch]
public static class MatchInfoGuidePatch
{
    ...
    private static RoleBehaviour GetRoleBehaviourFromBasis(CustomRoles role)
    {
        ...
        RoleTypes roleTypes = role.IsGhostRole() ? RoleTypes.GuardianAngel : role switch
        {
            ...
            // 上流は Judge→Prosecutor リネーム済みで CustomRoles.Prosecutor を使うが、当 fork の Judge 役職は未リネームなので CustomRoles.Judge のまま参照する。
            CustomRoles.Judge or CustomRoles.Mayor or CustomRoles.Dictator or CustomRoles.Swapper or CustomRoles.President => RoleTypes.Judge,
