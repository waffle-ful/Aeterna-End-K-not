...
internal static class MurderPlayerPatch
{
    ...
    public static void Postfix(PlayerControl __instance, [HarmonyArgument(0)] PlayerControl target, bool __state)
    {
        ...
        if (Overkiller.OverkillerDeadPlayerList.Contains(target.PlayerId) || (__instance.AmOwner && __instance.Is(CustomRoles.SoulCollector))) return;
...
internal static class ShapeshiftPatch
{
    ...
    public static bool ProcessShapeshift(PlayerControl shapeshifter, PlayerControl target)
    {
        ...
        {
            if (!Rhapsode.CheckAbilityUse(shapeshifter) || Stasis.IsTimeFrozen || TimeMaster.Rewinding || IntroCutsceneDestroyPatch.PreventKill)
            {
                shapeshifter.RpcRejectShapeshift();
                return false;
            }
...
internal static class ReportDeadBodyPatch
{
    ...
    ///   生存者を無作為に親にする / 赤ずきんの捕食 = 現場に死体だけ残して本人はマップ外へ運ばれる) まで巻き添えで通報
    ...
    public static bool Prefix(PlayerControl __instance, [HarmonyArgument(0)] NetworkedPlayerInfo target)
    {
        ...
        try
        {
            ...
            if (!target)
            {
                ...
                if (EvilBlender.On && EvilBlender.UseingId != byte.MaxValue)
                {
                    Notify("EvilBlenderMiniSabActive");
                    return false;
                }
    ...
    public static void AfterReportTasks(PlayerControl player, NetworkedPlayerInfo target)
    {
        ...
        ShrineMaiden.CurrentReportTarget = target;
...
internal static class ExitVentPatch
{
    public static void Postfix(Vent __instance, [HarmonyArgument(0)] PlayerControl pc)
    {
        ...
        VentOpener.OnAnyoneExitVent(pc);
...
internal static class EnterVentPatch
{
    public static void Postfix(Vent __instance, [HarmonyArgument(0)] PlayerControl pc)
    {
        ...
        VentOpener.OnAnyoneEnterVent(pc, __instance);
        VentMaster.OnAnyoneEnterVent(pc);
...
internal static class PlayerControlLocalSetRolePatch
{
    public static void Postfix(PlayerControl __instance, [HarmonyArgument(0)] RoleTypes role)
    {
        if (!AmongUsClient.Instance.AmHost && !GameStates.IsModHost)
        {
            ...
            {
                RoleTypes.Impostor => CustomRoles.ImpostorEndKnot,
                RoleTypes.Phantom => CustomRoles.PhantomEndKnot,
                RoleTypes.Shapeshifter => CustomRoles.ShapeshifterEndKnot,
                RoleTypes.Crewmate => CustomRoles.CrewmateEndKnot,
                RoleTypes.Engineer => CustomRoles.EngineerEndKnot,
                RoleTypes.Noisemaker => CustomRoles.NoisemakerEndKnot,
                RoleTypes.Scientist => CustomRoles.ScientistEndKnot,
                RoleTypes.Tracker => CustomRoles.TrackerEndKnot,
                RoleTypes.Detective => CustomRoles.DetectiveEndKnot,
                RoleTypes.Viper => CustomRoles.ViperEndKnot,
