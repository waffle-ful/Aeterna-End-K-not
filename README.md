...
End K not は**ホストのクライアントにだけ**入れれば動きます。参加者は Mod なしのまま、**682 の役職**を体験します。「導入してから来て」と言わなくていい。人を集めるときの摩擦がゼロです。公式サーバーでもカスタムサーバーでも、そのままフルに動きます。

### 部屋を、落とさせません。

通信エラーで切断されても、同じリージョン・同じ設定で**自動的に部屋を立て直します。** Among Us 本体がクラッシュ・ハングしても、付属の外部ウォッチドッグ（番犬）が検知して再起動し、部屋を復旧。放置したまま 24 時間でも回り続けます。「落ちたので今日は解散で」を、もう言わなくていい。
...
**End K not** は、[Endless Host Roles (EHR)](https://github.com/Gurge44/EndlessHostRoles) をベースとした Among Us の非公式個人フォークです。現在 **682 の役職**を実装しています。
