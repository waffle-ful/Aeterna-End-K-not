...
internal class Mathematician : RoleBase
{
    ...
    public static void Ask(PlayerControl pc, string num1Str, string num2Str)
    {
        try
        {
            ...
            LateTask.New(() => Utils.SendMessage(question, title: CustomRoles.Mathematician.ColoredTextByRole(Translator.GetString("Mathematician")), importance: MessageImportance.High), 0.2f, log: false);
        }
    ...
    public static void Reply(PlayerControl pc, string answerStr)
    {
        try
        {
            ...
            if (answer == State.Answer)
            {
                ...
                Utils.SendMessage(string.Format(Translator.GetString("MathematicianAnsweredString"), pc.GetRealName(), answer), title: CustomRoles.Mathematician.ColoredTextByRole(Translator.GetString("Mathematician")), importance: MessageImportance.High);
