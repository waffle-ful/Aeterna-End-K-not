...
internal class Bloodmoon : IGhostRole
{
    ...
    {
        if (target.Is(CustomRoles.Pestilence) || (target.Is(Team.Impostor) && pc.Is(Team.Impostor)) || !pc.RpcCheckAndMurder(target, true)) return false;
    ...
    {
        if (!ScheduledDeaths.TryGetValue(seer.PlayerId, out var entry)) return string.Empty;

        long timeLeft = Duration.GetInt() - (Utils.TimeStamp - entry.TimeStamp) + 1;
