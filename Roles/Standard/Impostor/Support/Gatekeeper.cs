...
public class Gatekeeper : RoleBase
{
    ...
    {
        // 会議中はプレイヤーが全員テーブルへ集められるので在室人数の集計に意味が無く、
        // 変化検知が誤発火して NotifyRoles + SyncRoleData を無駄撃ちする (fork 独自の Reliable レートゲート対策)。
        if (!pc.IsAlive() || GameStates.IsMeeting || !PerSecondUpdateScheduler.ShouldRunUpdate(pc.PlayerId)) return;
