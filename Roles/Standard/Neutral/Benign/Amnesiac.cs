...
public class Amnesiac : RoleBase
{
    ...
    private static void RememberRole(PlayerControl amnesiac, PlayerControl target)
    {
        ...
        hasValue |= sender.Notify(amnesiac, amneNotifyString, out sender);
        hasValue |= sender.Notify(target, CustomRoles.Amnesiac.ColoredTextByRole(GetString("AmnesiacRemembered")), out sender);
