...
internal class Predator : RoleBase
{
    ...
    {
        if (seer.PlayerId != target.PlayerId || seer.IsModdedClient() && !hud || Main.PlayerStates[seer.PlayerId].Role is not Predator { IsEnable: true } pt || pt != this) return string.Empty;
