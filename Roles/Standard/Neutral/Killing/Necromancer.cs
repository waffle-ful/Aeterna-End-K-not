...
internal class Deathknight : RoleBase
{
    ...
    public override bool OnCheckMurder(PlayerControl killer, PlayerControl target)
    {
        if (Necromancer.CanBeUndead(target) && Necromancer.PartiallyRecruitedIds.Contains(target.PlayerId))
        {
            ...
            hasValue |= sender.Notify(killer, CustomRoles.Necromancer.ColoredTextByRole(GetString("DeathknightRecruitedPlayer")), out sender);
            ...
            hasValue |= sender.Notify(target, CustomRoles.Necromancer.ColoredTextByRole(GetString("RecruitedByDeathknight")), out sender);
