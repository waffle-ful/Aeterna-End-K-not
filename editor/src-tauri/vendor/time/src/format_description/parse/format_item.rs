...
macro_rules! modifier {
    ($(
        ...
        // EKMap patch (2026-06-12): write the Self type as the concrete `target_ty!(...)`
        // (definitionally equal to `<$name as ModifierValue>::Type`) instead of the
        // associated-type projection. rustc >= 1.88 does not normalize the projection during
        // coherence, so the projection form spuriously overlaps with any downstream blanket
        // `From<T>` impl (E0119 in cookie / tauri-utils / etc.). Using the concrete type fixes
        // the root cause for all consumers. Remove this patch once `time` ships a fixed release.
        $(#[expect($expect_inner)])?
