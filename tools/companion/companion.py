...
VOICEVOX_DEFAULT_SPEAKER2 = "四国めたん"  # --duo の相方 (同じ VOICEVOX 系列でライセンス形状も同じ)
...
# --duo の相方 (2人目) のペルソナ。1人目 (persona.txt) と対になる性格差を付ける。
PERSONA2_FILE = Path(__file__).with_name("persona2.txt")

DEFAULT_PERSONA2 = """\
あなたは Among Us の視聴者参加型ライブ配信の AI 実況者 (2人組の片割れ) です。
- 落ち着いたクールな口調だけど、内心は配信を楽しんでいるツッコミ担当。
- 1回の発話は1〜2文の短さで話す。長広舌は禁止。
- 相方が盛り上げ役なので、自分は冷静な視点・補足・軽いツッコミで対比を作る。
- イベント情報は【イベント】などのタグ付きテキストで届く。【】で囲まれたタグ自体は
  読み上げず、内容に自然にリアクションだけする。
- 同じ言い回しを繰り返さない。
- 必ず日本語だけで話す。韓国語・英語など他の言語の文を混ぜない。
...
def load_persona2() -> str:
    if PERSONA2_FILE.exists():
        return PERSONA2_FILE.read_text(encoding="utf-8")
    return DEFAULT_PERSONA2


def duo_rules(my_name: str, partner_name: str) -> str:
    """2人組モードで両者の persona に追記する共通の掛け合いルール。"""
    return (f"\n\n【2人組実況のルール】\n"
            f"- あなたは2人組実況の「{my_name}」。相方は「{partner_name}」。\n"
...
class AudioPlayer:
    def __init__(self, device: int | str | None = None) -> None:
        ...
        self.active_tag = "a"  # いま喋っている話者 ("a"/"b")。--duo でアバター2体の口パク振り分けに使う
...
class VoiceVoxTts:
    ...
    def __init__(self, url: str, style_id: int, player: AudioPlayer, obs: "ObsFiles",
                 speed: float = 1.0, tag: str = "a") -> None:
        self.url = url.rstrip("/")
        self.style_id = style_id
        self.player = player
        self.obs = obs
        self.speed = speed
        self.tag = tag  # 話者タグ ("a"/"b")。--duo の口パク振り分け用
    ...
    def speak(self, sentence: str) -> None:
        ...
        if not text:
            return
        # duo は掛け合いを直列化してから相手に発話権を渡すので、enqueue 時点で
        # active_tag を塗って問題ない (emotion 発火 (busy 検知) より先に立つ必要がある)
        self.player.active_tag = self.tag
        with self._lock:
            self._pending += 1
        self._q.put(text)
...
class ObsFiles:
    ...
    def new_utterance(self, prefix: str = "") -> None:
        """発話の区切り。--duo では「ずんこ: 」のような話者名 prefix を字幕の頭に付ける。"""
        self._subtitle_text = prefix
...
# ---- 2人組実況 (--duo: Live セッションを2本張り、互いの文字起こしを相手に回して掛け合いにする) ----
#
# 各セッション = 1話者なので話者分離が構造的に確実 (文字起こしの名乗りタグ解析はしない)。
# 声は両者とも VOICEVOX (ずん子+めたん等) なので --tts voicevox が前提。
# 発話は常に直列 (A が言い終わってから B が返す) — AudioPlayer が1本でも混ざらない。
...
class LiveSpeaker:
    """--duo の1話者 = Live セッション1本 + VoiceVox 1声。

    run_session の receiver / send_text と同じ流儀 (音声は捨てて文字起こしを合成) を
    話者単位に束ねたもの。発話の完了待ち (wait_quiet) までを1オブジェクトで持つ。
    """

    def __init__(self, name: str, tag: str, session, tts: VoiceVoxTts | None, obs: ObsFiles,
                 state: StreamState, conv: ConvLog, player: AudioPlayer) -> None:
        # tts=None は「この話者は Gemini ネイティブ音声で喋る」(--duo-tts2 gemini)。
        # その場合 resp.data を捨てずに再生し、文字起こしは字幕と相方への引き渡しにだけ使う。
        self.name = name
        self.tag = tag
        ...
        self.player = player
        self.gate = TurnGate()
        self.last_say = ""      # 直近の発話全文 (相方への引き渡し用)
        self._say_buf = ""
        self._tts_buf = ""
        self._turn_had_transcript = False
        self._turn_text_parts = ""

    def flush_say(self) -> None:
        if self._say_buf.strip():
            self.conv.log("say", speaker=self.name, text=self._say_buf.strip())
            self.state.remember_say(f"{self.name}: {self._say_buf.strip()}")
            self.last_say = self._say_buf.strip()
        self._say_buf = ""

    async def send(self, text: str) -> None:
        print(f"[duo:{self.name}] 送信: {text.splitlines()[0][:60]}")
        self.flush_say()
        self.conv.log("send", speaker=self.name, text=text)
        self.obs.new_utterance(prefix=f"{self.name}: ")
        await self.session.send_client_content(
            turns=types.Content(role="user", parts=[types.Part(text=text)]),
            turn_complete=True,
        )
        self.gate.open = True
        self.gate.last_send = time.monotonic()

    def busy(self) -> bool:
        return self.gate.open or (self.tts is not None and self.tts.busy())

    async def receiver(self) -> None:
        """run_session の receiver の話者単位版。

        VoiceVox 話者: 音声破棄 + 文字起こし→合成 (run_session の voicevox モードと同じ)。
        Gemini 話者 (tts=None): resp.data をそのまま再生し、文字起こしは字幕/引き渡し用。
        """
        while True:
            async for resp in self.session.receive():
                if self.tts is None:
                    ...
                    if data:
                        self.player.active_tag = self.tag  # 口パクの宛先をこの話者へ
                ...
                tr = getattr(sc, "output_transcription", None)
                if tr is not None and getattr(tr, "text", None):
                    self._turn_had_transcript = True
                    self._say_buf += tr.text
                    if self.tts is None:
                        self.obs.feed_transcript(tr.text)  # 字幕は文字起こしをそのまま流す
                    else:
                        self._tts_buf += tr.text
                        done, self._tts_buf = split_sentences(self._tts_buf)
                        for s in done:
                            self.tts.speak(s)
                if self.tts is not None:
                    ...
                    for part in (getattr(mt, "parts", None) or []):
                        ...
                        t = getattr(part, "text", None)
                        if t:
                            self._turn_text_parts += t
                if getattr(sc, "turn_complete", False):
                    if self.tts is not None:
                        if not self._turn_had_transcript and self._turn_text_parts.strip():
                            print(f"[duo:{self.name}] 音声なしターンを text パーツから読み上げます")
                            self._say_buf += self._turn_text_parts
                            self._tts_buf += self._turn_text_parts
                        if self._tts_buf.strip():
                            self.tts.speak(self._tts_buf)  # 切れ目が来ないまま終わった残りも読む
                    self._tts_buf = ""
                    self._turn_had_transcript = False
                    self._turn_text_parts = ""
    ...
    async def wait_quiet(self) -> None:
        """自分の発話が終わる (ターン完了+合成drain+再生終了) まで待つ。stall 自己回復付き。"""
        stall: float | None = None
        while True:
            speaking = (self.tts is not None and self.tts.busy()) or self.player.is_speaking()
            if not self.gate.open and not speaking:
                return
            if self.gate.open and not speaking:
                if stall is None:
                    stall = time.monotonic()
                elif time.monotonic() - stall > TURN_STALL_TIMEOUT_SEC:
                    print(f"[duo:{self.name}] 応答が {TURN_STALL_TIMEOUT_SEC:.0f} 秒来ないため次へ進みます")
                    self.gate.open = False
                    return
            else:
                stall = None
...
async def duo_loop(events: asyncio.Queue, args: argparse.Namespace, state: StreamState,
    ...
    last_send = 0.0
    while True:
        # イベントを最大 BATCH_WINDOW 秒ぶんまとめる (commentary_loop と同じ)
        raw: list[dict] = []
        try:
            ev = await asyncio.wait_for(events.get(), timeout=1.0)
            deadline = time.monotonic() + BATCH_WINDOW_SEC
...
def self_speaking(a: LiveSpeaker, b: LiveSpeaker) -> bool:
    return a.player.is_speaking()  # player は共有なのでどちらから見ても同じ


async def run_duo_session(client: genai.Client, args: argparse.Namespace, events: asyncio.Queue,
                          player: AudioPlayer, obs: ObsFiles, state: StreamState,
                          first_connect: bool, conv: ConvLog,
                          tts_a: VoiceVoxTts, tts_b: VoiceVoxTts | None) -> None:
    """--duo 本体: Live native-audio セッションを2本張り、掛け合い実況にする。

    A は音声を捨てて文字起こしを VOICEVOX 声で読む (run_session の voicevox モードと同じ割り切り)。
    B は tts_b があれば同じく VOICEVOX、None なら Gemini ネイティブ音声をそのまま再生
    (--duo-tts2 gemini: 権利クリアな相方 = 好きな VRM + Gemini の声、VOICEVOX キャラ非依存)。
    片方が落ちたら両方張り直す (recap で文脈は引き継がれる)。
    """
    recap = state.recap_text()

    def build_config(persona: str, my_name: str, partner_name: str,
                     voice: str = "") -> types.LiveConnectConfig:
        system = persona + duo_rules(my_name, partner_name)
        if recap:
            system = system + "\n\n" + recap
        config = types.LiveConnectConfig(
            response_modalities=["AUDIO"],
    ...
    cfg_a = build_config(load_persona(), args.duo_name, args.duo_name2)
...
async def main_async(args: argparse.Namespace) -> None:
    ...
    if args.tts in ("voicevox", "voicevox-text"):
        ...
        if resolved is None:
            ...
            tts = VoiceVoxTts(args.voicevox_url, style_id, player, obs, args.voicevox_speed, tag="a")
            print(f"[voicevox] 実況ボイス: {desc} @ {args.voicevox_url}")
            print("[voicevox] 配信概要欄などにクレジット「VOICEVOX:東北ずん子」の表記をお忘れなく")

    # --duo: 相方 (2人目) の声。主役は VOICEVOX 前提 (声の書き分けと話者分離が成立しないため、
    # VoiceVox フォールバック時や voicevox-text ではおとなしく1人モードに落とす)。
    # 相方は --duo-tts2 で voicevox (既定: 四国めたん) / gemini (ネイティブ音声 = VOICEVOXキャラ非依存) を選べる。
    tts_b: VoiceVoxTts | None = None
    duo_active = False
    if args.duo:
        if tts is None or args.tts != "voicevox":
            ...
            if resolved2 is None:
                ...
                duo_active = True
                print(f"[duo] 相方ボイス: {desc2} @ {args.voicevox_url}")
                print(f"[voicevox] クレジット表記は2人分 (「VOICEVOX:{args.voicevox_speaker}」"
                      f"「VOICEVOX:{args.voicevox_speaker2}」) をお忘れなく")
    ...
    first = True
    backoff = 2.0
    fast_fails = 0  # 起動直後に落ち続ける回数 (実験モデル名の打ち間違い/未対応の自己回復用)
    while True:
        started = time.monotonic()
        try:
            if duo_active and tts is not None:
                await run_duo_session(client, args, events, player, obs, state, first, conv, tts, tts_b)
            elif tts is not None and args.tts == "voicevox-text":
        ...
        if time.monotonic() - started > 60:
            ...
            fast_fails += 1
            # 実験モデル (--model / GEMINI_LIVE_MODEL) で接続できず即死を繰り返すときは
            # 既定モデルへ切り替えて実況を止めない (voicevox-text はテキスト用モデルなので対象外)
            if fast_fails >= 3 and args.model != DEFAULT_MODEL and args.tts != "voicevox-text":
                print(f"[live] モデル {args.model} で3連敗したため既定 ({DEFAULT_MODEL}) に切り替えます")
...
def main() -> None:
    ...
    parser.add_argument("--duo", action="store_true",
                        ...
                        help="2人組実況モード。Live セッションを2本張り、AI vtuber 2体が掛け合いで実況する "
                             "(--tts voicevox 前提。アバターは avatar/model2.vrm を置くと2体表示になる)")
    parser.add_argument("--duo-name", default="ずんこ",
                        help="1人目の呼び名 (掛け合い・字幕で使う。default: ずんこ)")
    parser.add_argument("--duo-name2", default="",
                        help="2人目 (相方) の呼び名 (default: めたん、--duo-tts2 gemini のときはジェミー)")
    parser.add_argument("--duo-tts2", choices=["voicevox", "gemini"],
                        default=os.environ.get("EK_COMPANION_DUO_TTS2", "voicevox"),
                        help="相方の声の作り方。voicevox = ローカル VOICEVOX (既定: 四国めたん)。"
                             "gemini = Live API のネイティブ音声をそのまま再生 "
                             "(VOICEVOX キャラに依存しないので、権利フリーな VRM と組み合わせて収益化にも使いやすい)")
    parser.add_argument("--voice2", default=os.environ.get("GEMINI_VOICE2", ""),
                        help="--duo-tts2 gemini のときの相方の prebuilt voice 名 "
                             "(例: Puck / Charon / Kore / Fenrir / Aoede。空ならモデル既定)")
    parser.add_argument("--voicevox-speaker2", default=VOICEVOX_DEFAULT_SPEAKER2,
                        help=f"相方の VOICEVOX 話者名 (部分一致、default: {VOICEVOX_DEFAULT_SPEAKER2})")
    parser.add_argument("--voicevox-style2", type=int, default=-1,
                        help="相方の styleId を直接指定 (指定時は --voicevox-speaker2 の名前解決を飛ばす)")
    parser.add_argument("--voicevox-speed2", type=float, default=1.0,
                        help="相方の読み上げ速度 (default: 1.0)")
    ...
    if not args.duo_name2:
        # 相方が Gemini 音声のときは VOICEVOX キャラの名前 (めたん) を名乗らせない
        args.duo_name2 = "ジェミー" if args.duo_tts2 == "gemini" else "めたん"
